...
const FormField = ({ labelName, placeholder, inputType, isTextArea, value, handleChange, options, ...rest }) => {
      ...
      {isTextArea ? (
        ...
        >
          {options && options.map((option) => (
            <option key={option.value} value={option.value} className="bg-[#1e1e2d] text-white">
              {option.label}
            </option>
          ))}
        </select>
